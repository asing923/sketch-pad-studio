...
def saveToDisk(): 											
	...
	for curr_item in items:
		...
		if not flag:									
			...
			save_object["objects"].append(item_info)	
	if(debugOutput):		
		print(save_object)
...
avail_colors=["black","grey", "white","blue", "cyan3", "skyblue1","salmon","purple1","brown", "red", "green", "springgreen2", "yellow", "peachpuff3"]
counter=10
...
avail_figures=["Sketch", "Line", "Rectangle", "Ellipse", "Square", "Circle", "Polygon"]
counter=35
counte = 35
def figure_lambda(var):
	return lambda x: setFigure(var)
for curr in avail_figures:
	id = canvas.create_rectangle((counter, 10, counter+70, 30), fill="DodgerBlue4", tags=('buttons'))
...
entry1 = Entry (root) 
canvas.create_window(counter, 6, window=entry1, anchor=NW, tags=('buttons'))
counter = counter + 200;
...
def ur_lambda(var):
	...
	return lambda x: setSaved(var)
for curr in undo_redo:
    
	id = canvas.create_rectangle((counte, 35, counte+70, 55), fill="red", tags=('buttons'))
	canvas.tag_bind(id, "<Button-1>", saving_lambda(curr))
	id = canvas.create_text((counte+5, 35), text=curr, tags=('buttons'), anchor=NW, fill="white")
	canvas.tag_bind(id, "<Button-1>", saving_lambda(curr))
	counte = counte + 75;
